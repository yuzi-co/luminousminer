...
benchmark::KernelParams parseKernelParams(boost::json::object const& obj, benchmark::KernelParams const& defaults)
{
    benchmark::KernelParams p{ defaults };
    if (obj.contains("loop"))
    {
        p.loop = static_cast<uint32_t>(obj.at("loop").to_number<uint64_t>());
    }
    if (obj.contains("threads"))
    {
        p.threads = static_cast<uint32_t>(obj.at("threads").to_number<uint64_t>());
    }
    if (obj.contains("blocks"))
    {
        p.blocks = static_cast<uint32_t>(obj.at("blocks").to_number<uint64_t>());
    }
    return p;
}

benchmark::AlgoConfig parseAlgoConfig(boost::json::object const& obj, benchmark::AlgoConfig const& defaultAlgo)
{
    benchmark::AlgoConfig algo{ defaultAlgo };

    if (obj.contains("enabled"))
    {
        algo.enabled = obj.at("enabled").as_bool();
    }
    if (obj.contains("loop"))
    {
        algo.defaults.loop = static_cast<uint32_t>(obj.at("loop").to_number<uint64_t>());
    }
    if (obj.contains("threads"))
    {
        algo.defaults.threads = static_cast<uint32_t>(obj.at("threads").to_number<uint64_t>());
    }
    if (obj.contains("blocks"))
    {
        algo.defaults.blocks = static_cast<uint32_t>(obj.at("blocks").to_number<uint64_t>());
    }
...
benchmark::VendorConfig parseVendorConfig(boost::json::object const& obj, benchmark::VendorConfig const& defaultVendor)
{
    benchmark::VendorConfig vendor{ defaultVendor };

    if (obj.contains("enabled"))
    {
        vendor.enabled = obj.at("enabled").as_bool();
    }
    if (obj.contains("device_index"))
    {
        vendor.deviceIndex = static_cast<uint32_t>(obj.at("device_index").to_number<uint64_t>());
    }
...
benchmark::Config benchmark::Config::makeDefault()
{
    ...
    cfg.nvidia.algorithms["blake2b"] = { true,
                                         { 10u, 128u, 1024u },
                                         { { "lm1", { 10u, 128u, 1024u } }, { "lm2", { 10u, 128u, 1024u } } } };
