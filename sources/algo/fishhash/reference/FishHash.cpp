...
#include "blake3.h"  // shared vendored reference (crypto/reference/blake3, via blake3_ref)

// NOTE (LuminousMiner vendor patch): upstream FishHash.cpp #includes keccak.c and
// blake3.c directly, which compiles C as C++ (enum|int errors) and risks duplicate
// symbols. We instead compile keccak.c / blake3.c as C translation units (see
// sources/algo/fishhash/CMakeLists.txt) and only pull the headers here.
