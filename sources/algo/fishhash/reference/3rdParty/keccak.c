...
/* LuminousMiner vendor patch: drop `inline` so an external `keccak` symbol is always
 * emitted (the Windows cross toolchain compiles this .c as C++, where an unused inline
 * definition emits no out-of-line symbol -> link error `undefined symbol: keccak`). */
