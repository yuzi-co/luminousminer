...
namespace
{
    ...
    {
        algo::hash256 out{};
        EXPECT_TRUE(algo::randomx::calculateHash(key.data(), key.size(), input.data(), input.size(), out));
        return algo::toHex(out);
    }
}
...
TEST_F(RandomXKatTest, reKeyRebuildsCache)
{
    ...
    EXPECT_EQ(algo::toHex(a), "639183aae1bf4c9a35884cb46b09cad9175f04efd7684e7262a0ac1c2f0b4e3f");

    ASSERT_TRUE(hasher.init("test key 001", 12u, true));
    algo::hash256 b{};
    hasher.hash("This is a test", 14u, b);
    EXPECT_EQ(algo::toHex(b), "351562e03304417135a5f8946164e8900f24fd45b444c092b029b439ddbc1450");
}
